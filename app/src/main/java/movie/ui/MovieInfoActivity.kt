...
/** The launch function is a static method that can be used to start the MovieInfoActivity from an Activity
 instance. It takes parameters such as activity (the current activity)
 movies (a list of ParcelableMovie instances), and selectedMovieId
 */
...
internal class MovieInfoActivity : AppCompatActivity() {
    ...
    private inner class ScreenSlidePagerAdapter(
        ...
        override fun createFragment(position: Int) = MovieInfoFragment.newInstance(
            movieImageIds[position].imdbId,
            movieImageIds[position].posterUrl
        )
    }
}
